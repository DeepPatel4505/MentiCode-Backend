...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    if (!incomingRefreshToken) {
        return res
            .status(200)
            .json(new ApiResponse(200, null, "Refresh token is missing"));
    }
...
const githubLogin = (req, res) => {
    ...
    const url =
        ...
        `&scope=read:user user:email` + // repo access
...
const githubCallback = asyncHandler(async (req, res) => {
    ...
    if (state?.startsWith("connect:")) {
        ...
        if (existingGithubUser && existingGithubUser.id !== userId) {
            throw new ApiError(400, "GitHub already linked to another account");
        }
...
const githubConnect = (req, res) => {
    ...
    const params = new URLSearchParams({
        ...
        redirect_uri: callbackUrl,
        state: `connect:${req.user.id}`, // encode user
    });
